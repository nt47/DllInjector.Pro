...
namespace DllInjector.Pro
{
    public partial class frmMain : Form
    {
        ...
        public frmMain()
        {
            ...
            this.comboBox_Method.SelectedIndex = 0;
        }
        ...
        private void btnInject_Click(object sender, EventArgs e)
        {
            ...
            if (g_InjectorType == 0)
            {
                if (CLR.Injector.Inject((int)this.lsvProcesses.SelectedItems[0].Tag, this.txtDll.Text) != 0)
        ...
        private static class NativeMethods
        {
            ...
            public static string GetProcessFileName(Process p)
            {
                ...
                uint bufferLength = (uint)builder.Capacity + 1;
                IntPtr ptr = OpenProcess(ProcessAccessFlags.All, false, p.Id);
                if (!QueryFullProcessImageName(ptr, 0, builder, ref bufferLength))
                {
                    return string.Empty;
                }
            ...
            public static IntPtr SafeOpenProcess(int PID)
            {
            ...
            public static string WhichPlatform(int PID)
            {
                var hProcess = new SafeHandle(NativeMethods.OpenProcess(NativeMethods.ProcessAccessFlags.All, false, PID));
                if (hProcess.IsInvalid)
                    return "[?]";
                bool isX86;
                NativeMethods.IsWow64Process(hProcess, out isX86);
                if (isX86)
                    return "[32]";
        ...
        private int g_InjectorType = 0;
        private void comboBox_Method_SelectedIndexChanged(object sender, EventArgs e)
        {
            // 当用户选择不同选项时执行操作
            //MessageBox.Show("选中了：" + comboBox_Method.SelectedItem.ToString());
            if (comboBox_Method.SelectedItem.ToString() == "ManualMap")
            {
                g_InjectorType = 0;
            }
            else if (comboBox_Method.SelectedItem.ToString() == "Standard")
            {
                g_InjectorType = 1;
            }
        }
